...
- **テーマ別リーディング**: 仕事・創作活動、体調・メンタル、人間関係、金運・生活 の4テーマ。各テーマは「状況」「課題」「アドバイス」をポジションごとに分けず、`<div class="reading-text">` 1つに収める一続きの文章として書く。カード名（節制の正位置が示す通り、など）は文中に自然に織り込む。状況・課題は前半で1〜2文ずつ端的に触れ、後半のアドバイスを状況・課題を踏まえた具体的な行動として厚めに書く（複数の具体的アクションを含めてよい。詳細は「占いとして機能させるための指針」参照）

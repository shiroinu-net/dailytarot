...
- **テーマ別リーディング**: 仕事・創作活動、体調・メンタル、人間関係、金運・生活 の4テーマ、各テーマで3ポジション分のテキストを書く。カード名の後に `（正位置）` または `（逆位置）` を付ける。状況・課題は1〜2文で端的に、アドバイスは状況・課題を踏まえた具体的な行動として書く（詳細は「占いとして機能させるための指針」参照）
